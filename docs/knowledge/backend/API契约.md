...
> **状态**：已冻结（2026-09-05；产品决策 P-1~P-9 与跨文档契约裁决 D-01~D-12 已确认；2026-09-06 增补 §3.5/3.6 执行调用日志端点，见 [任务执行调用日志方案](../../requirements/archive/任务执行调用日志方案.md)）
> **对应文档**：[后端方案（§2.6 API 契约）](../../requirements/archive/后端方案.md)｜[产品需求分析](../产品需求分析.md)
...
| 市场 | GET /api/v1/market-data/stocks/{symbol}/bars | 第二阶段（2026-09-14 增补） |
...
### 8.1 ~~资产目录 GET /api/v1/market-assets~~（已删除，证券市场数据库统一方案决策 4）

目录端点与 market_assets 表已删：平台资产目录前端写死（`MARKET_INDEX_CATALOG` 11 指数常量：US 3 + KS11 + CN 7，组序 US → KR → CN 固化；2026-09-14 US/KR 上线后 KOSDAQ 无可用源从目录移除）；后端不再提供目录查询。
...
| symbol | string | 交易代码（如 000001.SH / .INX / KS11） |
| name | string | 指数名称 |
...
```json
{
  "data": {
    ...
    "indicators": {
      "ma": [
        { "period": 5,  "values": [3348.1, 3350.2] },
        { "period": 10, "values": [3346.5, 3348.9] },
        { "period": 20, "values": [3342.8, 3345.1] },
        { "period": 60, "values": [3331.2, 3334.0] }
      ],
      "boll": { "period": 20, "k": 2.0, "mid": [3342.8, 3345.1], "upper": [3401.2, 3404.6], "lower": [3284.4, 3285.6] },
      "macd": { "fast": 12, "slow": 26, "signal": 9, "dif": [12.34, 13.01], "dea": [10.21, 10.88], "hist": [4.26, 4.26] }
    },
    "source": "tushare",
    "as_of": "2026-09-04",
...
- items 按 rank ASC；`rank`、`algorithm_version` 是唯一热点解释来源，前端不自行计算；`hotness_reason` 现场计算版恒 NULL（LLM 理由生成未实现，heat_v2 引入时再定）。
...
- STALE 可展示旧数据并标注榜单日期与延迟（freshness 由 sector_daily 最新 trade_date 与交易日历比较判定）；market≠CN 返回空态（503 语义整体删除，决策 13）。
...
```json
{
  "data": {
    ...
    "source": "dc",
    "source_updated_at": "2026-09-11T18:00:00Z",
    "freshness_status": "FRESH"
  },
  "meta": { "request_id": "...", "schema_version": "v1" }
}
