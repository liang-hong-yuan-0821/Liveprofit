...
> **状态**：已完成（2026-09-12）——**被 [技术指标数据源切换方案.md](技术指标数据源切换方案.md) 吸收**（用户拍板"并入"，D6）
> **吸收说明**：本方案 3.1 的 ema/macd 纯函数自算设计与"技术指标不自算"决策（2026-09-12）矛盾，未实施；3.2 契约接线与 3.3 前端副图设计被吸收后照搬实施——macd 数值取自 idx_factor_pro 入库数据（hist 为上游 macd_bfq 原值），契约与前端骨架不变。详见吸收方案 3.3。
> **进度**：3/3 步骤（随吸收方案实施完毕）
> **关联文档**：[K线指标叠加方案.md](K线指标叠加方案.md)（已归档，MA/BOLL 同模式扩展）、[API契约.md](../../knowledge/backend/API契约.md) §8.2
...
- K 线图已叠加 MA5/10/20/60 + BOLL(20,2)（[K线指标叠加方案](K线指标叠加方案.md)，2026-09-06 完成归档）：后端补窗口（WARMUP_DAYS=120 自然日）纯 Python 计算，`BarsData.indicators` 并列数组契约，前端 CandlestickChart 主图渲染 + dataZoom 缩放。
